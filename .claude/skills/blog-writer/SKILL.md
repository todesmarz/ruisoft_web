...
description: |
  ...
  - 「〜について技術ブログを作成して」
  - 「〜の解説記事を書いて」
  - 「〜について深掘りしてブログ記事（Markdown）にして」
  - 「〜についてまとめ記事を書いて」
  「ブログ」「解説記事」「まとめ記事」「技術記事」「技術ブログ」などの言葉が含まれる依頼では必ずこのスキルを使うこと。
  ※「前日のニュースを収集して」は daily-news スキルを使用すること。本スキルはテーマ調査に基づく解説記事の執筆を担う。
  ※「logs フォルダの記事をマッピングして」は blog-mapper スキルを使用すること。本スキルは新規記事の執筆を担う。
...
**視覚要素の配置ルール（必須）:**
主要セクション（テーマの主役の紹介／検証／考察／標準セクション群）は、それぞれ最低1つの視覚要素（概念イラストSVG・構造図SVG・HTMLテーブルのいずれか）を含めること。文章だけで完結するセクションを作らない。
加えて、セクション内であっても説明が長い・内容が難しいと判断した箇所には都度イラストを追加する（配置トリガーの詳細はStep 4.6参照）。
...
## Step 4.6: 概念イラストSVG（新設・必須）

構造図（フロー・関係図）とは別に、**抽象概念を可愛らしいキャラクター/オブジェクトで比喩・擬人化して直感的に理解させるイラスト**を作成する。目的は「読んで理解する」から「見て理解する」への転換。

### 作成方針

1. **記事内で使った比喩をそのまま絵にする**
   - 本文で「JITコンパイルは即興通訳者のようなもの」と書いたら、実際に通訳者らしき可愛いキャラクターと吹き出しでその動きを描くSVGを作る
   - 比喩とイラストを一致させることで、文章とイラストが補完し合う関係にする

2. **単純な図形の羅列にしない**
   - NG：四角と矢印だけの構造図を「イラストです」と言い張る
   - NG：無機質でかたい印象のアイコン止まり
   - OK：丸みを帯びたフォルム・大きめの目や表情・柔らかい配色で「かわいい」と感じさせるキャラクター/オブジェクト
   - 完全な写実は不要。マスコット的にデフォルメされたキャラクターで十分（むしろそのほうが望ましい）

3. **配置トリガー（固定セクションだけでなく内容ベースで判断）**
   下記のいずれかに該当する箇所には、その都度イラストを配置する：
   - **テーマの主役の紹介セクション**：主役のマスコット化イラスト（必須）
   - **説明が長くなる箇所**：1セクション内の説明文が目安400字を超える場合、要点を1枚のイラストに要約して添える
   - **内容が難しい・抽象的な箇所**：専門用語や仕組みの説明で、比喩だけでは伝わりにくいと判断した場合
   - **ビフォーアフターや対比を示したい箇所**：検証・考察・比較セクションなど
   - 執筆中に「ここは文章だけだと読者が離脱しそうだ」と感じた箇所は、セクションの種類によらず追加してよい

### かわいいイラストのスタイルガイド

- **キャラクター化**：概念・技術・データなどを、丸顔＋大きな目＋シンプルな体のマスコットキャラクターとして擬人化する
- **フォルム**：角を丸める、直線より曲線を多用する、パーツを大きめ・デフォルメ気味に描く
- **表情**：困り顔・笑顔・驚き顔など感情を持たせ、状況（エラー時／成功時など）に応じて表情を変える
- **配色**：パステル系の柔らかい色を基調にし、頬の赤み・ハイライトなど「かわいさ」を加える差し色を1〜2色使う
- **小物・エフェクト**：吹き出し、キラキラ、汗マーク、電球（ひらめき）などの記号的エフェクトで感情や状況を補足する
- 1枚のイラストで伝える情報は1テーマに絞る（詰め込みすぎない）

```html
<svg viewBox="0 0 640 240" xmlns="http://www.w3.org/2000/svg"
     style="max-width:100%;font-family:sans-serif;">
  <!-- 背景：淡いパステルカラーで場面を表現 -->
  <!-- キャラクター：丸みのあるフォルム＋大きな目＋表情で可愛さを表現（円=顔、角丸四角/楕円=胴体等） -->
  <!-- 表情・吹き出し・エフェクト（キラキラ/汗/電球等）でストーリー性と感情を演出 -->
  <!-- ラベル：日本語テキストは font-family:sans-serif を指定 -->
</svg>
```

### 構造図SVG（既存・継続使用）

フロー・構造・関係性を正確に伝えたい場合は、従来通り矢印・ボックスの構造図を使う。
概念イラストと構造図は用途が異なるため、両方が必要なセクションでは両方配置してよい。
...
- [ ] 専門用語の初出に補足が付いているか
- [ ] **主要セクション（テーマ紹介／検証／考察／標準セクション群）それぞれに視覚要素（概念イラストSVG・構造図SVG・HTMLテーブルのいずれか）があるか**
- [ ] **概念イラストSVGが単なる図形の羅列ではなく、比喩・ストーリー性・かわいらしさ（丸み・表情）を持っているか**
- [ ] **説明が長い箇所・内容が難しい箇所に、都度イラストによる補足があるか**
...
2. `logs/` フォルダに保存する
3. 記事の概要（文字数・含まれるセクション・使用した視覚要素の数）を一言で添える
