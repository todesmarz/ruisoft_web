...
## 実務で使える形にするためのチェックリスト

次のステップ記事は、読むだけだと「知っている」で止まりがちです。実務レベルへ近づけるため、提出前に次を確認してください。

- [ ] 例外処理で `except Exception:` の握りつぶしをしていない
- [ ] ファイル操作で `with open(...)` を使い、クローズ漏れがない
- [ ] 辞書・リストのどちらを使うべきか理由を説明できる
- [ ] 内包表記を多段にしすぎず、可読性を優先している
- [ ] クラスは「データと振る舞いが一体のとき」に限定して使っている
...
- 悪い例：条件分岐や変換を1行に詰め込み、後から読めない
- 改善：2段階に分けて、変換とフィルタを分離する

### 2) 例外処理で原因を消す

- 悪い例：`except:` で全部握りつぶして `pass`
- 改善：想定する例外型を指定し、ログやメッセージで原因を残す

### 3) クラス化の早すぎ問題

- 悪い例：関数2つ程度の小処理まで無理にクラス化
- 改善：まず関数で作り、状態と振る舞いの結合が必要になってからクラス化

---

## 最後に：学習順序で迷ったら

迷ったら次の順番で学習すると、理解が崩れにくくなります。
...
この順序は「まず動かす → 壊れにくくする → 継続開発できる形にする」という実務の流れに一致しています。
