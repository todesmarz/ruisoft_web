...
> Gemini CLIを「何でもやる社員」ではなく「曖昧な箇所だけを見る判定係」として組み込み、再実行でき、止められ、監査できる小さな業務自動化を作る方法をまとめる。

## 🤖 主役は「万能エージェント」ではなく、判断部品だ
...
Gemini CLI自体にも、長い自律実行を完全に任せるハーネス（モデルに道具、状態管理、権限制御、再試行を与える実行基盤）として気になる点はある。公式リファレンスでも、ヘッドレス実行には「ターン上限超過」の終了コードがあり、対話確認を要するポリシーは非対話時に拒否扱いになる。また、ワークスペース階層のポリシーは現時点で機能しないという警告も明記されている。これは欠陥をあげつらう話ではない。**止まる前提で外側のプログラムを設計する理由**だ。

## 🧪 仮説：工程を二つに割れば、使える範囲は広がる
...
## 🔬 検証：CLIに任せてよい場所、任せない場所

公式文書を機能ごとに読むと、短い判定部品として使う材料はそろっている。`-p` による一回実行、標準入力、`json` またはストリーミングJSON、標準終了コードがある。サンドボックスはホストから操作を隔離でき、ポリシーエンジンはツールを許可・拒否・確認に振り分けられる。拡張にはプロンプト、MCPサーバー、コマンド、フック、サブエージェント、スキルを同梱できる。
