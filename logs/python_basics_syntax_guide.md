...
## 学習を定着させる30分演習メニュー（毎日用）

「読んだだけ」で終わると、翌週には忘れます。以下の30分メニューを1〜2週間回すと、基本記法が手に馴染みます。

1. **10分：写経**  
   本記事の `if` / `for` / `関数` のサンプルを手で打つ（コピペ禁止）。
2. **10分：改造**  
   変数名・条件・出力文を自分の題材（家計簿、筋トレ記録など）に変える。
3. **10分：説明**  
   「このコードは何をしているか」を3行でコメントに書く。

### 演習課題（初学者向け）

- 課題A：1〜100のうち3の倍数だけ表示する
- 課題B：点数リストから平均点を計算し、60点未満の人数を表示する
- 課題C：名前と年齢を受け取り、成人/未成年を判定する関数を作る

> ポイントは「正解すること」より「なぜその書き方にしたかを言語化すること」です。
...
<table>
  ...
  <tbody>
    <tr><td>IndentationErrorが出る</td><td>インデント幅が不揃い</td><td>4スペースで統一、Tab混在を避ける</td></tr>
    <tr><td>TypeErrorが出る</td><td>文字列と数値を混在計算した</td><td><code>int()</code>/<code>str()</code>で型変換する</td></tr>
    <tr><td>for文が1回しか動かない</td><td>ループ外に処理を書いている</td><td>インデント範囲を見直す</td></tr>
    <tr><td>関数が値を返さない</td><td><code>return</code>を書いていない</td><td>返したい値を明示的にreturnする</td></tr>
  </tbody>
</table>
...
このページの内容が実装できるようになったら、次は以下の順で進むのがおすすめです。

1. リスト・辞書を使ったデータ管理
2. 例外処理（try/except）で壊れにくいコードへ
3. ファイル入出力でデータを保存
4. テスト導入（pytest）で再現可能な品質管理

「文法が全部わかってから作る」ではなく、**小さく作りながら必要な文法を増やす**学び方が最短です。
